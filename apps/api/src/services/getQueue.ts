...
export default async function getQueue({ supabase, userId }: GetMeArgs) {
    ...
    if (postsError) {
        throw new Error(`Failed to fetch queue: ${postsError?.message}`);
    }
    ...
    if (commentIds.length > 0) {
        ...
        if (currentUserLikedCommentsError) {
            throw new Error(`Failed to fetch liked comments: ${currentUserLikedCommentsError?.message}`);
    ...
    const formattedPosts = posts?.map(post => {
        return {
            ...
            comments: post.post_comment.map((comment) => {
                        const commentProfile = firstOrNull(comment.profile);
                        return {
                            id: comment.id,
